...
export default function ProgramCatalog({ programs, onSelectProgramToApply }) {
  ...
  programs.forEach(p => {
    ...
    if (p.themeColor || p.bgImage || p.textColor) {
      ...
      customStyles.push(`
        ...
        body .program-card.program-${p.id} .program-degree { color: ${textColor} !important; background: rgba(0, 0, 0, 0.4); border: 1px solid rgba(255, 255, 255, 0.2); border-radius: 50px; padding: 4px 14px; display: inline-block; width: fit-content; text-transform: uppercase; font-size: 11px; font-weight: 700; letter-spacing: 1px; }
        body .program-card.program-${p.id} .program-name { text-transform: capitalize; margin-top: 8px; color: ${textColor} !important; }
        body .program-card.program-${p.id} .card-body p, 
        body .program-card.program-${p.id} .card-body .spec-label, 
        body .program-card.program-${p.id} .card-body .spec-val { color: ${textColor} !important; }
      `);
    }
  });
