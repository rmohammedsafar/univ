...
// In Firebase Functions v2, configuration is usually handled via .env files 
// or Firebase Secret Manager instead of functions.config().
...
exports.sendEmailTrigger = onDocumentCreated("email/{docId}", async (event) => {
  ...
  const { type, fullName, studentEmail, programTitle, highestQual, country, state, phone, inquiryName, inquiryEmail } = data;

  try {
    if (type === 'inquiry') {
      if (!inquiryName || !inquiryEmail) {
        console.error('Missing required parameters: inquiryName or inquiryEmail');
        return null;
      }
          ...
          <div style="font-family: Arial, sans-serif; background-color: #ffffff; color: #333333; padding: 20px; border-radius: 12px; max-width: 580px; width: 100%; margin: 0 auto; border: 1px solid #d4af37; box-shadow: 0 4px 15px rgba(0,0,0,0.05); box-sizing: border-box;">
            ...
            <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-collapse: collapse; background-color: #faf8f5; border: 1px solid #d4af37; border-radius: 8px; margin: 20px 0;">
              ...
              <tr>
                ...
                <td style="padding: 10px 14px; border-bottom: 1px solid #eee; font-size: 14px; font-weight: bold; color: #111; word-break: break-word;">${phone}</td>
              </tr>` : ''}
              ${programTitle ? `
              <tr>
      ...
      console.log(`Inquiry confirmation sent to ${inquiryEmail} and registrar.`);
      return null;
    }

    if (!studentEmail) {
      console.error('Missing required parameters: studentEmail');
      return null;
    }
    ...
    console.log(`Email dispatched to ${studentEmail} and Registrar.`);
    return null;
  } catch (error) {
